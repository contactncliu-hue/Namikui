...
let currentTab = 'attendance'; // Active tab: 'attendance', 'ranking', or 'members'

// Load saved data or initialize defaults
let eventDates = JSON.parse(localStorage.getItem('amt_eventDates')) || [];
let members = JSON.parse(localStorage.getItem('amt_members')) || [];

function saveData() {
    localStorage.setItem('amt_eventDates', JSON.stringify(eventDates));
    localStorage.setItem('amt_members', JSON.stringify(members));
    if (currentUser.role) {
...
function render() {
    ...
    // Sort members by total attendance
