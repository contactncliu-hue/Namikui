...
// Guard: if the Supabase CDN script hasn't loaded before this file runs,
// `supabase` will be undefined and everything below would crash silently.
if (typeof supabase === 'undefined') {
    console.error(
        'Supabase library not found. Make sure this script tag is in your HTML BEFORE app.js:\n' +
...
// Load data from Supabase Cloud on startup (Syncs across devices)
async function loadDataFromCloud() {
    try {
        ...
        console.error('Error loading from cloud:', err.message || err);
    }
}

// Save data to Supabase Cloud (Pushes changes to all devices)
async function saveData() {
    ...
    try {
        const { error: err1 } = await db.from('app_data')
            .upsert({ id: 1, key: 'eventDates', value: eventDates }, { onConflict: 'id' });
        if (err1) console.error('Save Error (Events):', err1.message);

        const { error: err2 } = await db.from('app_data')
            .upsert({ id: 2, key: 'members', value: members }, { onConflict: 'id' });
        if (err2) console.error('Save Error (Members):', err2.message);
    } catch (err) {
        console.error('CRITICAL ERROR saving to cloud:', err.message || err);
    }
}
...
function render() {
    ...
    members.sort((a, b) => (b.attended || 0) - (a.attended || 0));
    ...
    if (currentTab === 'attendance') {
        if (typeof renderAttendanceTable === 'function') {
            ...
            console.warn('renderAttendanceTable() is not defined. Add it in another script file, or the Attendance tab will stay empty.');
        }
...
// --- INITIALIZATION ---
// Wait for DOM to be ready before touching elements/loading data.
